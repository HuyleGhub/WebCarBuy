...
const ImportExportComponent = () => {
  ...
  const handleFileImport = async (event:any) => {
    ...
    try {
      const response = await fetch('api/xe/import', {
        method: 'POST',
        body: formData
      });
   ...
   const handleExport = async () => {
      try {
        const response = await fetch(`api/xe/export?format=${exportFormat}`);
  ...
  const handleReport = async () => {
    try {
      const response = await fetch('api/xe/report');
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-3 py-2 h-10 border rounded-lg bg-white text-xs"
            >
